package com.domain.lesson14IOSerialization;
...
public class IOLesson {
    public static void main(String[] args) {
        // io пакет
        // работает с char / byte
        // два потока InputSteam / OutpStream

        // любой тип данных или объект нужно привести к char / byte

        // InputStream
        // OutputStream
//// МЕТОДЫ
        File file = new File("io.txt");
        file.exists(); // tru/false существует файл или нет
        file.isDirectory(); // tru/false  папка или нет?
        file.isFile();
        file.canRead();
        file.canWrite();
        try{
...
//            System.out.println(readByteArray(file));
    ...
    public static void writeToFile(File file, boolean append) throws IOException {
        try(FileOutputStream fileOutput = new FileOutputStream(file, append)){// закрытие потока происходит до  блока catch
    ...
    public static String readByteArray(File file) throws IOException {
        ...
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream()
            ...
            System.out.println(fileInputStream.available());
             ...
             int data;// для проверки закончился файл или нет
             while ((data = fileInputStream.read(buf))> 0){ // когда файл закончится read вернет -1 и цикл  прервется
    ...
    public static String readFromSeveralFiles(File ...files)throws IOException {
        ...
        try (FileInputStream input1 = new FileInputStream(files[0]);
            ...
            // если файлов больше 2х нужен Vector<>
...
//            RandomAccessFile позв. работать с серединой файла, позв. открывать файл в реж.запись и реж. чтения
            // Дома разобрать

           // своя реализация InputStream  OutputStream
